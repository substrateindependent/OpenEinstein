"""Unit tests for verification agent inconsistency detection."""
...
def test_verification_agent_detects_planted_inconsistency(tmp_path: Path) -> None:
    ...
    report = agent.run(
        ...
        claims=[
            ...
            {"key": "ghost_free", "value": False, "source": "calc-b"},
            {"key": "stable", "value": True, "source": "calc-a"},
        ],
    )
    assert report["inconsistent"] is True
    assert report["review_required"] is True
    assert len(report["issues"]) == 1
    assert report["issues"][0]["key"] == "ghost_free"
